...
for img_file in tqdm.tqdm(test_img_list):
    ...
    import pudb;pu.db
    inds_gray = cv2.cvtColor(inds_rgb, cv2.COLOR_BGR2GRAY)
    thresh = threshold_otsu(inds_gray)
    binary = inds_gray > thresh
    binary = 255 * binary
    binary = binary.astype('uint8')
    kernel = np.ones((7,7), np.uint8)
    dilated = cv2.dilate(binary, kernel, iterations=1)
    import pudb; pu.db
    #binary(np.where(bonary == True)) = 255
    #binary(np.where(bonary == False)) = 0 
    cv2.imwrite( os.path.join(args.input, 'result/') + os.path.basename(img_file), inds_rgb )
    cv2.imwrite( os.path.join(args.input, 'result/') + 'gray/' + os.path.basename(img_file), dilated)
