...
check_point = ModelCheckpoint("./my_model_resnet_smooth_label"+name+".h5", monitor="val_loss", save_best_only=True)
...
plt.xlabel('epoch')
plt.savefig('pretrained_model_results_w3_lr2_smooth_label'+name+'.png')
...
early = EarlyStopping(monitor="val_loss", mode="min", patience=10)
check_point2 = ModelCheckpoint("./my_model_resnet_fine_tune_smooth_label"+name+".h5", monitor="val_loss", save_best_only=True)
#reduce = ReduceLROnPlateau(monitor="val_loss", factor=0.1, patience=6)
...
plt.title('Training and Validation Loss')
plt.xlabel('epoch')
plt.savefig('fine_tuned_model_results_w3_lr2_smooth_label'+name+'.png')
...
for i in range(9):
  ...
  plt.axis("off")
  plt.savefig('smooth_label_resnet'+name+'.png')
